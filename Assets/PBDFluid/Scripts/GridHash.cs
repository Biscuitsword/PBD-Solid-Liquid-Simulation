...
namespace PBDFluid
{
    ...
    public class GridHash : IDisposable
    {
        ...
        public int ExtraParticles { get; private set; }
        ...
        public GridHash(Bounds bounds, int numParticles, float cellSize, int extraParticles = 0)
        {
            TotalParticles = numParticles;
            ExtraParticles = extraParticles;
            CellSize = cellSize;
            InvCellSize = 1.0f / CellSize;
            //Debug.Log("ExtraParticles : " + ExtraParticles);
            Groups = (TotalParticles + ExtraParticles) / THREADS;
            if ((TotalParticles + ExtraParticles) % THREADS != 0) Groups++;
            ...
            IndexMap = new ComputeBuffer((TotalParticles + ExtraParticles), 2 * sizeof(int));
            Table = new ComputeBuffer(size, 2 * sizeof(int));

            m_sort = new BitonicSort((TotalParticles + ExtraParticles));
        ...
        public void Process(ComputeBuffer particles)
        {
            ...
            m_shader.SetBuffer(m_hashKernel, "Extra", particles);//unity 2018 complains if boundary not set in kernel
        ...
        // TODO: Add multiple to hash process
        ...
        public void Process(ComputeBuffer particles, ComputeBuffer boundary, ComputeBuffer extra)
        {
            int numParticles = particles.count;
            int numBoundary = boundary.count;
            int numExtra = extra.count;
            //Debug.Log("Grid hash NumExtra : " + extra.count + "/ " + ExtraParticles);

            if (numParticles + numBoundary != TotalParticles || numExtra != ExtraParticles)
                throw new ArgumentException("numParticles + numBoundary != TotalParticles");

            m_shader.SetInt("NumParticles", numParticles);
            m_shader.SetInt("TotalParticles", TotalParticles);
            m_shader.SetInt("ExtraParticles", ExtraParticles);
            m_shader.SetFloat("HashScale", InvCellSize);
            m_shader.SetVector("HashSize", Bounds.size);
            m_shader.SetVector("HashTranslate", Bounds.min);
            ...
            m_shader.SetBuffer(m_hashKernel, "Particles", particles);
            m_shader.SetBuffer(m_hashKernel, "Boundary", boundary);
            m_shader.SetBuffer(m_hashKernel, "Extra", extra);
            m_shader.SetBuffer(m_hashKernel, "IndexMap", IndexMap);
